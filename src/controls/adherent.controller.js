...
module.exports.updateAdherent = async(req, res) =>{
    try {
        const updateAdherent = await Adherents.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
...
module.exports.findAdherentByFilter = async(req, res)=>{
    try {
        ...
        const adherents = await Adherents.find({ profession: profession, ville: ville })
